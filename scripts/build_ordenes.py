...
.chip.enodc{color:#0b5c8a;background:#e1f0fb;border:1px solid #b6dcf5}
.chip.falta{color:#b26a00;background:#fdf3e0;border:1px solid #f0d9a8}
.chip.dupodc{color:#c0392b;background:#fdecea;border:1px solid #f3c2bc}
input.q.enodc{background:#f2f9ff;border-color:#7fb6e0}
tr.enodc td{background:#fbfdff}
...
function bindUndo(){var u=document.getElementById('undoseed');if(!u)return;
 u.onclick=function(e){e.preventDefault();
  ...
  for(var k in SEED_0809){delete edits[k];}
  esave();localStorage.removeItem(SKEY);renderOrden();draftbar();};}
function rowHTML(r,tm,noSug){
  ...
  if(faltaODC)chips+=' <span class="chip falta">\u2795 NO está en la orden 08/09</span>';
  // Aviso fuerte: esta en la orden 08/09 PERO ya viene en camino -> se pediria doble.
  if(enODC&&t>0&&!out&&sug(r,tm)===0)chips+=' <span class="chip dupodc">\u26A0\uFE0F Ya vienen '+t+' pzs en camino — pedirlo sería doble</span>';
  ...
  return '<tr class="'+(noSug?'gris ':'')+(enODC?'enodc':'')+'" data-k="'+r.k+'">'+
   '<td>'+img(r.i)+'</td>'+
   ...
   '<td class="num"><input class="q'+(s>0?' hot':'')+(enODC?' enodc':'')+(!out&&edits[r.k]!=null?' edited':'')+'" type="number" min="0" step="1" value="'+s+'"'+(out?' disabled title="Los OUTLET no se piden"':'')+'></td>'+
